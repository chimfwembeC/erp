...
const EmployeeForm = ({ employees }) => {
  ...
  const handleSubmit = (e) => {
    ...
    Swal.fire({
      title: 'Are you sure?',
      text: "You are about to create a new employee.",
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        const method = 'post';
        const url = route('hrm.employees.store');
        ...
        router[method](url, formData, {
          ...
          onError: (error) => {
            Swal.fire('Error', `There was an issue creating the employee.`, 'error');
          },
        });
      }
    });
        ...
        <div className="p-6 bg-white shadow-md rounded-lg col-span col-span-2">
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <div className="flex justify-end">
              <Button type="submit" label={'Create Employee'} icon="pi pi-check" className="p-button-success bg-primary text-white p-2 rounded-lg" />
            </div>
          </form>
        </div>
      </div>
