...
const Edit = ({ inventoryMovement,warehouses, products }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    router.put(route('inventory.inventory_movements.update', inventoryMovement.id), formData, {
      onSuccess: () => {
        Swal.fire('Updated!', 'The inventorymovements has been Updated successfully.', 'success').then(() => {
  ...
  return (
    <AppLayout title="Edit inventory movements">
      <Breadcrumb items={items} />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <SectionTitle
        title="Edit New inventory movements"
      ...
      <div className="p-6 bg-white shadow-md rounded-lg col-span col-span-2">
        <h1 className="text-2xl font-bold mb-4">Edit inventory movements</h1>
        <form onSubmit={handleSubmit} className="grid gap-4">
          ...
          <Button type="submit" label="Update Warehouse" icon="pi pi-check" className="p-button-success bg-primary text-white p-2 rounded-lg" />
          </div>
        </form>
      </div>
      </div>
