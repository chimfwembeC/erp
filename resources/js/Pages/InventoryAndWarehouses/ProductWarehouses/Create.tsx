...
const Create = ({ warehouses, products }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    router.post(route('inventory.product-warehouses.store'), formData, {
      onSuccess: () => {
  ...
  return (
    <AppLayout title="Add product to warehouse">
      ...
      <div className="p-6 bg-white shadow-md rounded-lg col-span col-span-2">
        <h1 className="text-2xl font-bold mb-4">Add product to warehouse</h1>
        <form onSubmit={handleSubmit} className="grid gap-4">
          ...
          <Button type="submit" label="Add product to warehouse" icon="pi pi-check" className="p-button-success bg-primary text-white p-2 rounded-lg" />
          </div>
        </form>
      </div>
      </div>
