        ...
        <AppLayout title="Product Management">
            ...
            <div className="px-4 py-8">
                ...
                {isGridView ? (
                            ...
                            {products.slice(first, first + rows).map((product) => (
                                <div key={product.id} className="bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                                    <div className="p-6">
                                        ...
                                        <div className="mt-4">
                                            <p className="text-sm text-gray-500"><span className="font-semibold">Price:</span> ${product.price}</p>
                                            <p className="text-sm text-gray-500"><span className="font-semibold">Stock:</span> {product.stock}</p>
                                        </div>
                                    </div>
                                    <div className="flex justify-end p-4 border-t border-gray-200">
                                        <Button icon="pi pi-eye" className="p-button-text text-blue-600" onClick={() => router.get(`/inventory/products/${product.id}`)} />
                                        <Button icon="pi pi-pencil" className="p-button-text text-yellow-500" onClick={() => router.get(`/inventory/products/${product.id}/edit`)} />
                                        <Button icon="pi pi-trash" className="p-button-text text-red-600" onClick={() => handleDelete(product.id)} />
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <DataTable value={products} paginator rows={10} stripedRows className="p-datatable-lg">
