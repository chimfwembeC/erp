...
const EditPurchaseOrder = ({ suppliers, purchaseOrder }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      const result = await Swal.fire({
        title: 'Are you sure?',
        text: 'Do you want to update this purchase order?',
        icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        router.put(route('accounting.purchase-orders.update', purchaseOrder.id), purchaseOrderData, {
          onSuccess: () => {
            Swal.fire(
              'Updated!',
          ...
          onError: () => {
            Swal.fire(
              'Error',
              'There was an issue updating the purchase order.',
              'error'
            );
          },
    ...
    <AppLayout title="Edit Purchase Order">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <SectionTitle
          ...
          description="Update the details of the purchase order."
        />
