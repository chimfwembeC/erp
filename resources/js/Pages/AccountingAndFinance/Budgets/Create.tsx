  ...
  const handleSubmit = async e => {
    ...
    if (validateForm()) {
      const result = await Swal.fire({
        title: 'Are you sure?',
        text: 'Do you want to create this budget?',
        icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        router.post(route('accounting.budgets.store'), budgetData, {
          onSuccess: () => {
            Swal.fire(
              'Created!',
          ...
          onError: error => {
            Swal.fire(
              'Error',
              'There was an issue creating the budget.',
              'error',
            );
            setErrors(error);
          },
  ...
  return (
    <AppLayout title="Create Budget">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <SectionTitle
          title="Create Budget"
          description="Fill in the details below to create a new budget."
        />
