...
import { Home, User, FileText, Settings, Bell, Menu, X, ChevronDown, ChevronUp } from 'lucide-react'; // Example icons
...
export default function Layout({ title, children }: LayoutProps) {
  const [sidebarOpen, setSidebarOpen] = useState(false); // Handle sidebar toggle on mobile
  const [employeeDropdownOpen, setEmployeeDropdownOpen] = useState(false); // Employee dropdown state
  const [payrollDropdownOpen, setPayrollDropdownOpen] = useState(false); // Payroll dropdown state
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-100">
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 z-30 h-full w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
          }`}
      >
        ...
        <div className="relative">
          ...
          {activeDropdown === 'settings' && (
            <div className="absolute right-0 top-12 mt-2 w-48 bg-white shadow-lg rounded-lg py-2">
              ...
              <div className="p-4 hover:bg-indigo-50">
                <button
